/********************************** (C) COPYRIGHT
 ******************************** File Name          : system_ch32v20x.c Author
 *: WCH Version            : V1.0.0 Date               : 2021/06/06 Description
 *: CH32V20x Device Peripheral Access Layer System Source File. For HSE = 32Mhz
 *(CH32V208x/CH32V203RBT6) For HSE = 8Mhz (other CH32V203x) Copyright (c) 2021
 *Nanjing Qinheng Microelectronics Co., Ltd. SPDX-License-Identifier: Apache-2.0
 *********************************************************************************/
#include "ch32v20x.h"

/*
 * Uncomment the line corresponding to the desired System clock (SYSCLK)
 * frequency (after reset the HSI is used as SYSCLK source). If none of the
 * define below is enabled, the HSI is used as System clock source.
 */
...
uint32_t SystemCoreClock =
    SYSCLK_FREQ_HSE; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_48MHz_HSE
uint32_t SystemCoreClock =
    SYSCLK_FREQ_48MHz_HSE; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_56MHz_HSE
uint32_t SystemCoreClock =
    SYSCLK_FREQ_56MHz_HSE; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_72MHz_HSE
uint32_t SystemCoreClock =
    SYSCLK_FREQ_72MHz_HSE; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_96MHz_HSE
uint32_t SystemCoreClock =
    SYSCLK_FREQ_96MHz_HSE; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_120MHz_HSE
uint32_t SystemCoreClock =
    SYSCLK_FREQ_120MHz_HSE; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_144MHz_HSE
uint32_t SystemCoreClock =
    SYSCLK_FREQ_144MHz_HSE; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_48MHz_HSI
uint32_t SystemCoreClock =
    SYSCLK_FREQ_48MHz_HSI; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_56MHz_HSI
uint32_t SystemCoreClock =
    SYSCLK_FREQ_56MHz_HSI; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_72MHz_HSI
uint32_t SystemCoreClock =
    SYSCLK_FREQ_72MHz_HSI; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_96MHz_HSI
uint32_t SystemCoreClock =
    SYSCLK_FREQ_96MHz_HSI; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_120MHz_HSI
uint32_t SystemCoreClock =
    SYSCLK_FREQ_120MHz_HSI; /* System Clock Frequency (Core Clock) */
#elif defined SYSCLK_FREQ_144MHz_HSI
uint32_t SystemCoreClock =
    SYSCLK_FREQ_144MHz_HSI; /* System Clock Frequency (Core Clock) */
#else
 ...
 *
 * @brief   Sets System clock frequency to 96MHz and configure HCLK, PCLK2 and
 * PCLK1 prescalers.
 *
 * @return  none
 */
 ...
 *
 * @brief   Sets System clock frequency to 120MHz and configure HCLK, PCLK2 and
 * PCLK1 prescalers.
 *
 * @return  none
 */
static void SetSysClockTo120_HSE(void) {
  ...
  if (HSEStatus == (uint32_t)0x01) {
     ...
     *  CH32V20x_D8W-PLL configuration: PLLCLK = HSE/2 * 15 = 240 MHz
     * (HSE=32MHZ)
     */
    RCC->CFGR0 &=
        (uint32_t)((uint32_t) ~(RCC_PLLSRC | RCC_PLLXTPRE | RCC_PLLMULL));
 ...
 *
 * @brief   Sets System clock frequency to 144MHz and configure HCLK, PCLK2 and
 * PCLK1 prescalers.
 *
 * @return  none
 */
static void SetSysClockTo144_HSE(void) {
  ...
  if (HSEStatus == (uint32_t)0x01) {
     ...
     *  CH32V20x_D8W-PLL configuration: PLLCLK = HSE/4 * 18 = 144 MHz
     * (HSE=32MHZ)
     */
    RCC->CFGR0 &=
        (uint32_t)((uint32_t) ~(RCC_PLLSRC | RCC_PLLXTPRE | RCC_PLLMULL));
